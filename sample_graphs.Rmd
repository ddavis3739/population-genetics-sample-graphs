...
- y-axis can be reformated after running function using scale_y_continuous()
- be sure to use expand = c(0,0) and limits = c(0, x) within to drop bars down to x-axis
- error bars can be added in function when replicate data is avaible

```{r echo = F, message=FALSE}
require(ggplot2); require(dplyr); require(ggsci); require(gridExtra); require(grid)
...
### F in relation to number of ROHs

- minor formatting can still be done
- need to test on data
...
-- 2 y-axis is a little wonky, maybe just do two seperate plots in one panel?
- need to look at dataset to better optimize
